...
@router.get(
    ...
    response_model=Page[AtletaOutSimplified],
)
async def query(
    ...
    # Usar paginação do SQLAlchemy.
    page_result = await sqlalchemy_paginate(db_session, query_stmt, params)

    # Converter para AtletaOutSimplified.
    items = [AtletaOutSimplified.model_validate(atleta) for atleta in page_result.items]

    return Page.create(items=items, total=page_result.total, params=params)
